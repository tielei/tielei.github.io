...
* 如果将军之间使用口头消息(oral messages)，也就是说，消息被转述的时候是可能被篡改的，那么要对付m个叛徒，至少3m+1个将军（其中至少2m+1个将军是忠诚的）。
* 如果将军之间使用签名消息(signed messages)，也就是说，消息被发出来之后是无法伪造的，只要被篡改就会被发现，那么对付m个叛徒，只需要至少m+2个将军，也就是说至少2个忠诚的将军（如果只有1个忠诚的将军，显然这个问题没有意义）。
...
我们平常能接触到的BFT的一个典型的例子，就是区块链。一个区块链网络是一个完全开放的网络，其中的矿工节点(miner)是可以自由加入自由退出的。当然，这些节点有可能是恶意的，区块链网络在设计的时候必须要考虑这个问题。这实际上就是典型的「拜占庭将军问题」。
...
在区块链中使用的消息属于签名消息，具体体现在：每一个区块中的交易(transaction)都是由交易发起者的密钥签名过的，其它接收区块消息的节点都能通过公钥进行验证。那么，这属于上述第二种情况，难道说区块链网络中忠诚节点的数目没有要求？显然不是这样。比如在比特币网络中，要求恶意节点不能掌握多于50%的计算力。
