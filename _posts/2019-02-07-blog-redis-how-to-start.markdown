...
* 最后将命令执行结果发送给客户端的过程，由`beforeSleep`来触发。它检查输出buffe中有没有需要发送给客户端的执行结果数据，如果有的话，会调用`writeToClient`尝试进行发送。如果一次性没有把数据发送完毕，那么还需要再向事件循环中注册一个写IO事件回调`sendReplyToClient`，在恰当的时机再次调用`writeToClient`来尝试发送。如果还是有剩余数据没有发送完毕，那么后面会由`beforeSleep`回调来再次触发这个流程。

---

redisCommandTable就在server.c中
根据自己看代码过程中的疑问和过程。
