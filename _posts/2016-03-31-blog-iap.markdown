...
* 使用最新的App Receipt验证方式，不要再使用旧的transactionReceipt的验证方式。
...
因此，最终我们决定还是使用已经过期的SKPayment的paymentWithProduct:来创建一个SKPayment实例。这样可以做到先将商店购买UI快速展示出来。
...
因此，如果有条件，建议给App Store服务器的验证请求加上国际代理（可以使用HTTP的CONNECT方法）。
