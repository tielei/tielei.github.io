...
这张图出自去年的一篇综述论文[2]，展现了近几年LLM技术的进化树。从图中可以看出，按照LLM的模型架构不同，大体可以分成三个大的技术分支：
* Encoder-Only;
* Encoder-Decoder;
...
这种**每次生成下一个单词**的工作方式，就是所谓的「**自回归**」(auto-regressive)。不管是对于Transformer还是LLM来说，这都是至关重要的一个特性。之所以按照这种方式工作，其实也好理解：
* 每生成一个新的单词，显然都需要参考前面已经生成的所有单词，才能有助于形成一个合适的完整句子。人类做翻译的时候也是类似的过程。
* 输出的句子，它所包含的单词数量是预先未知的，只能在「步步为营」的生成过程中动态地确定句子长度。
...
现在，结合前面英文翻译德文的例子，我们把这个图的每个局部解释一下。

#### Tokenizer

不管是句子还是单词，计算机都没法直接处理。计算机只能处理数字。因此，输入的文本在送进Transformer模型之前，会先经过一个Tokenizer，把文本转换成模型能够处理的数字（向量形式）。
...
每个Encoder Layer到底做了什么呢？这里面关键的一个机制是**自注意力 (self-attention)**；
* 一个**自注意力 (self-attention)**。这也是Transformer成功的关键因素之一。
...
* 这种关系的紧密程度，是受上下文影响的。比如，在前面第 (1) 个句子中，***it***与***animal***的关系更为紧密；而在前面第 (1) 个句子中，***it***则与***street***的关系更为紧密。
...
首先，***Output Embedding***模块的输入，一定要注意。这里的输入，其实是最终生成的输出。那么，在最开始模型还没有输出任何token的时候，这里输入什么呢？答案是：标识序列起始状态的特殊token，也就是`<bos>`。

一旦有了token输入，整个生成过程就可以执行起来了，每次生成一个新的token。在Transformer模块图中，图的下半部分展示了生成过程中的一个时刻：根据上一步已经生成的序列“Ein kleines Mädchen”，正在生成下一个token：「schaut」。图中的数字`[2, 5, 69, 27]
`以及`[100]`，表示相应token在德文字典中的索引编号。

***Output Embedding***的功能与前面的***Input Embedding***一样，都是把token从数字表示，转换为向量表示（512维）。也就是到了图中 (G) 的位置。

后面经过三个Decoder Layer，每个Decoder Layer中又包含两个注意力机制：
* 一个**自注意力 (self-attention)**；
* 一个**交叉注意力 (corss-attention)**

这里的**自注意力**，是用来描述**生成的序列**的（德文）；而前面Encoder中的**自注意力**，是用来描述**输入序列**的（英文）。这中间有个非常关键的区别：生成的过程需要遵循**因果关系**。也就是说，生成下一个token的时候，它只能**注意到 (attend to)**之前已经生成的token；所以，对于已经生成的序列来说，Decoder Layer对这个序列进行处理的时候，序列中每个token也都应该保持跟生成时一样的逻辑，它只能**注意到 (attend to)**在它之前的token。

而Encoder中的**自注意力**却允许，序列中的每个token，都可以**注意到 (attend to)**所有的token（包括在它之前和它之后的）。

Decoder中的**自注意力**，为了遵循因果关系，在计算上需要构建一个mask矩阵：

[<img src="/assets/images_transformer/transformer_target_mask.jpg" style="width:400px" alt="mask矩阵" />](/assets/images_transformer/transformer_target_mask.jpg)
...
* 通过Decoding过程进行生成，没经过一层Decoder Layer，都执行一次**自注意力**和一次**交叉注意力**的过程。
...
**关于动图的说明**：图中每个圆点表示一个向量，也就是某个token在Encoder或Decoder某一层的内部表示。图中的运动弧线，表示沿着**自注意力**或**交叉注意力**指导的方向上，token中信息的流动方向。在前半部分的Encoding过程中，自上而下，是经历了三个Encoder Layer；而在后半部分的Decoding过程中，则是自下而上，经历了三个Decoder Layer。

### 小结

以上本文的所有描述，基本上都是基于一个已经训练好的Transformer来讨论的。在这个训练好的Transformer中，注意力权重参数已经有了恰当的值。而这些注意力权重参数的得来，则依赖Transformer的训练过程。

除了训练过程，还有一些重要的细节，由于篇幅原因，本文也没涉及到。比如，多头注意力机制 (Multi-Head Attention) ，位置编码 (Positional Encoding) ，用于计算注意力权重的q、k、v，等等。如果想把这些讨论清楚，还需要引入很多额外的其他信息，我们后面有机会再说。
...
所有这些因素加起来，使得研究人员能够把互联网级的数据灌进模型，并能在有限的时间内获得结果。结合Decoding的过程，LLM彻底把***Predict Next Token***发扬光大了。基于此，OpenAI发展起了影响深远的***Scaling law***[4]。
