...
虽然我们从具体的模型开始，但是，等我们真正地深入讨论之后就会发现，具体的模型其实并不重要，重要的是蕴含在整个过程里的设计思想。它们才是我们更需要关注的东西。所以，在组织这篇文字的时候，我内心始终抱着这样一个信念：讲清楚具体模型的运作原理并不是本文的真正目的（虽然它对于明确本文的讨论方向起着至关重要的作用），理清背后的整体脉络和思想才是更重要的。总之，等你读完本文就知道了，模型并不是本文的重点。
...
严格来说，优化并不属于机器学习的范畴。它们之间的关联在于：机器学习的模型训练（即模型的求解）会用到优化理论。所以，接下来我们的目标就是讨论清楚这个关联是怎么来的。

把机器学习问题转换成一个优化问题的关键在于，优化的目标怎么表达？由于机器学习的整套理论是基于概率的，因此，机器学习模型求解的优化目标，也是基于概率来表达的。

我们前面说过，站在机器学习的视角，这个世界的一切都是概率。说实话，这个看似「很酷」的论断有时候并不是很好理解。

大概很多人都听说过，机器学习可以解决分类问题，也可以解决回归问题。举个例子，假如我们要预测明年北京市五环附近的房价，那么这是一个回归问题。如果说房价是一个随机变量，它满足一个概率分布，这个还比较容易理解。因为这跟我们的直观感觉比较接近。明年北京五环的房价大概率会落在某个价位区间内，但谁也不能确定到底是多少。然而，一个分类问题，如果说它也是基于概率的，也许就不是那么容易理解的了。比如图像识别问题，给你一张图片，让机器学习模型来判断里面包含什么物体（是汽车、人，还是动物，等等）。你也许会说，每一张图片都是确定性的，里面有什么物体也都是确凿的事情，这跟概率又有什么关系呢？但在机器学习的世界里，这也是个概率问题。所有像素组合在一起，形成一张怎样的图片，这也是满足一定的概率分布的（可以将各个像素的值看成一个多维随机变量，它们满足一个联合概率分布）。只要是人类产生的图片，那么在这样的一个概率分布下，所有像素组合在一起形成一张有意义的图片的概率是比较大的，而形成一张完全无法理解的图片的概率则是很小的。

实际上，我们观测到的任何数值（比如人的身高、图片的像素值、每天的股票价格、每天的气温、每年的房价，等等），它们背后都对应一个概率分布，称为真实分布 (true distribution)。每当我们在现实世界观测到一个具体的数值时，就相当于从这个真实分布中采样 (sampling) 得到了一个样本数据。对应到前面的图片像素值满足的概率分布，就可以看成是这样一个真实分布。而当我们观看到一张具体的图片时，相当于从这个真实分布中采样到了一个样本数据。所有样本数据可以组成一个数据集，那么也可以说这个数据集是由这个真实分布产生 (generate) 的。

这似乎是说：当我们拿到了成千上万张图片的时候，我们就可以说，这些图片是从某个概率分布产生出来的。听起来有点像「无限猴子理论」，就是说“如果许多猴子任意敲打打字机键，最终可能会写出大英博物馆所有的书”。你可能会说，这概率也太低了，但事实上这些图片不是猴子产生的，也不是“任意敲打打字机”产生的，而是由人创作出来的，所以，从这个概率分布产生出来的像素的组合，非常可能就是一张有意义的图片。

当我们接受了这种概率观点之后，就可以考虑一下本小节前面的问题了：机器学习问题是怎么表达成一个优化目标的。这存在着两条思维路径：
1. 函数估计；
2. 数据拟合。
...
前面刚刚讨论过，任何观测到的数值背后都对应一个真实概率分布。假设这个概率分布可以记为*P*(***x***, *y*)，其中***x***是输入变量，*y*是输出变量（称为label或target）。那么一个预测问题可以看成是寻找一个函数*F*<sup>\*</sup>(***x***)，使得它与*y*最接近，这样就可以用*F*<sup>\*</sup>(***x***)作为*y*的近似值（或预测值）。这样的一个预测问题，可以称为函数估计（Function estimation）。

对于随机变量，怎么才算「最接近」呢？我们可以假设有一个损失函数（loss function），记为*L*(*y*,*F*(***x***))。那么，让这个损失函数在整个分布上的均值（也就是期望）最小，就可以定义为「最接近」。用公式表达就是：

[<img src="/assets/photos_gbdt/equation_Fstar.png" style="height:32px" alt="F(x)" />](/assets/photos_gbdt/equation_Fstar.png)

上面的损失函数的形式，常见的有平方误差*(y-F)<sup>2</sup>*，绝对值误差*\|y-F\|*，等等。

上面的公式看起来已经表示成一个优化问题的目标了，而且，目前涉及到的都还只是基于概率的纯数学计算。但是，上面函数估计问题的求解，依赖一个前提条件：真实分布*P*(***x***, *y*)是已知的。通常实际情况并不是这样的。因此，我们就需要从纯数学回到现实，用数据来近似表达真实分布。

假设从真实分布*P*(***x***, *y*)中采样*N*次，得到了*N*个数据样本：{***x***<sub>*i*</sub>, *y<sub>i</sub>*}，其中*i*=1,2,...,*N*。这*N*个数据样本代表了一个经验分布(empirical distribution)。现在就可以用这个经验分布（样本数据）来近似表达真实分布了。上面的优化公式可以写为：

[<img src="/assets/photos_gbdt/equation_Fstar_approximation.png" style="height:52px" alt="F(x)近似" />](/assets/photos_gbdt/equation_Fstar_approximation.png)
